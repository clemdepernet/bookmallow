...
## Écarts constatés à l'implémentation (v1.1.0)

- **LibriVox (§6.1)** : la requête par auteur est toujours émise, sans `^` (même résultat qu'avec `^`, et meilleur rappel). L'API répond HTTP 404 quand une requête n'a aucun résultat : c'est traité comme « aucun résultat » pour chacune des deux requêtes, et comme `not_found` pour la résolution d'un livre.
- **Internet Archive (§6.3)** : le champ `track` n'ordonne les pistes que si toutes les pistes retenues en ont un (il n'est souvent hérité que par quelques dérivés) ; sinon tri naturel sur le nom. Le titre de chapitre vient du champ `title` du fichier. Couverture : nom contenant `itemimage`/`cover`/`front`, puis JPEG/PNG d'origine, jamais un spectrogramme, une forme d'onde ou une vignette, sinon `services/img/<id>`. L'identifiant est validé (`^[A-Za-z0-9][A-Za-z0-9._-]{0,99}$`, sinon `not_found`) et encodé dans les URL.
- **Assemblage (§6.3)** : deux passes. La première encode audio, métadonnées et chapitres sans couverture (la progression `-progress` vient de cette passe : avec un flux image dans la même sortie, ffmpeg rapporte la position minimale entre flux, bloquée à l'unique image). S'il y a une couverture, une seconde passe la remuxe en `-c copy` (`-map 0:a -map 1:v`, la piste texte de chapitres de la première passe étant refusée par le muxer ipod) vers `Titre.part.cover.m4b`, qui remplace le fichier partiel.
- **Assemblage, pistes distantes** : chaque ligne `file 'http(s)://…'` de la liste ffconcat est suivie de `option rw_timeout 30000000`, `option reconnect 1`, `option reconnect_on_network_error 1`, `option reconnect_on_http_error 429,5xx` et `option reconnect_delay_max 60` (délai de lecture, reconnexion sur coupure et erreurs transitoires).
- **Torrent (§6.4)** : les états `moving`, `checkingResumeData`, `allocating` et `metaDL` ne sont pas considérés comme terminés, même à 100 %. Un `content_path` hors de `QBT_PATH_MAP` échoue en `no_audio` ; `QBT_PATH_MAP` sans `:` est une erreur de configuration. Le client qBittorrent se reconnecte une fois sur 401/403 avant de conclure à `qbt_auth`.
